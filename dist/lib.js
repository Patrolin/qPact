let lib = new function() {
	...
	t.TimeInterval = class {
		...
		valueOf() {
			let {y: e, o: r, d: n, h: i, m: s, s: o, ms: a} = this;
			return e * t.YEAR + r * t.MONTH + n * t.DAY + i * t.HOUR + s * t.MINUTE + o * t.SECOND + a;
		}
		modify(e = "") {
			let r = this;
			o(r, e);
		...
		toString() {
			let {y: e, o: r, d: n, h: i, m: s, s: o, ms: a, sign: u} = this, l = {};
			e && (l.years = e);
			r && (l.months = r);
			n && (l.days = n);
			i && (l.hours = i);
	...
	function o(t, e) {
		let r = 1;
		for (let [n, i, o, a, u, l] of e.matchAll(/(\d+)([a-z]+)|([a-z]+)(\d+)|([+-])/g)) if (i) {
			let e = s.indexOf(o);
			if (0 > e && o in t) t[o] += r * i; else {
				let n = t.getDate(), s = t.getDay();
				if (e != s) {
					n += r > 0 ? e - s + 7 : e + s - 15;
					i--;
				}
		...
		} else u && a in t ? t[a] = +u : l && (r = '+' == l ? 1 : -1);
	}
	t.assoc_keys = function(t) {
		let e = t[Symbol.iterator];
		if (e) {
			let r = 'entries' === e.name, n = Array.from(e.call(t));
			return [ r, $1r ? n.map(([t]) => t) : n.map((t, e) => e) ];
		}
		{
			let e = [];
	...
	t.describe = function(t) {
		return `${t != n ? `${t.constructor.name}: ` : ''}${t}`;
	};
